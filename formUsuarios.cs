...
namespace SistemaDeVentas
{
    public partial class formUsuarios : Form
    {
        ...
        private void formUsuarios_Load(object sender, EventArgs e)
        {
            ...
            {
                comboBoxRol.Items.Add(new OpcionCombo() { Valor = rol.IdRol, Texto = rol.Descripcion });
            }
        ...
        private void iconButtonGuardarCambios_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxContraseña.Text != textBoxConfirmarClave.Text)
            {
                MessageBox.Show("Las contraseñas no coinciden", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (textBoxContraseña.Text == "" || textBoxCorreo.Text == "" || textBoxDocumento.Text == "" || textBoxNombreCompleto.Text == "")
            {
                MessageBox.Show("Uno de los campos esta vacío, no se aceptan campos sin valor.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (u.IdUsuario == 0)
            {
                int idUsuarioGenerado = new CN_Usuario().Registrar(u, out mensaje);
                if (idUsuarioGenerado != 0)
                {
                   ...
                   new object[]
                   {
                    ...
                    MessageBox.Show("Usuario creado con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
            ...
            else
            {
                ...
                if (editado)
                {
                    ...
                    row.Cells["Contraseña"].Value = textBoxContraseña.Text;
                    ...
                    MessageBox.Show("Usuario editado con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
        ...
        private void iconButtonEliminar_Click(object sender, EventArgs e)
        {
            ...
            bool eliminado = new CN_Usuario().Eliminar(u, out mensaje);
            if (eliminado)
            {
                dataGridViewDataUser.Rows.RemoveAt(Convert.ToInt32(textBoxIndiceGrid.Text));
                MessageBox.Show("Usuario eliminado con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        private void dataGridViewDataUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridViewDataUser.Columns[e.ColumnIndex].Name == "buttonSeleccionar")
            {
                ...
                if (index >= 0)
                {
                    ...
                    textBoxId.Text = dataGridViewDataUser.Rows[index].Cells["IdUsuario"].Value.ToString();
                    ...
                    textBoxContraseña.Text = dataGridViewDataUser.Rows[index].Cells["Contraseña"].Value.ToString();
                    textBoxConfirmarClave.Text = dataGridViewDataUser.Rows[index].Cells["Contraseña"].Value.ToString();

                    foreach (OpcionCombo item in comboBoxRol.Items)
                    {
                        if (item.Valor.ToString() == dataGridViewDataUser.Rows[index].Cells["IdRol"].Value.ToString())
                        {
