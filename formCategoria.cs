...
namespace SistemaDeVentas
{
    public partial class formCategoria : Form
    {
        ...
        private void formCategoria_Load(object sender, EventArgs e)
        {
            ...
            foreach (Categoria c in listaCategoria)
            {
                dataGridViewDataUser.Rows.Add(
                    ...
                    c.Descripcion,
                    c.Estado == true ? 1 : 0,
                    c.Estado == true ? "Activo" : "No activo"
                    }
                    );
            }
        }
        ...
        private void iconButtonGuardarCambios_Click(object sender, EventArgs e)
        {
            ...
            if (u.IdCategoria == 0)
            {
                int idCategoriaGenerado = new CN_Categoria().Registrar(u, out mensaje);
                if (idCategoriaGenerado != 0)
                {
                   ...
                   new object[]
                   {
                    ...
                    MessageBox.Show("Categoría creada con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
            ...
            {
                editado = new CN_Categoria().Editar(u, out mensaje);
                if (editado)
                {
                    DataGridViewRow row = dataGridViewDataUser.Rows[Convert.ToInt32(textBoxIndiceGrid.Text)];
                    row.Cells["IdCategoria"].Value = textBoxId.Text;
                    ...
                    MessageBox.Show("Categoría editada con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
        ...
        private void iconButtonEliminar_Click(object sender, EventArgs e)
        {
            ...
            bool eliminado = new CN_Categoria().Eliminar(u, out mensaje);
            if (eliminado)
            {
                dataGridViewDataUser.Rows.RemoveAt(Convert.ToInt32(textBoxIndiceGrid.Text));
                MessageBox.Show("Categoría eliminada con exito.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        private void dataGridViewDataUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridViewDataUser.Columns[e.ColumnIndex].Name == "buttonSeleccionar")
            {
                ...
                if (index >= 0)
                {
                    ...
                    textBoxId.Text = dataGridViewDataUser.Rows[index].Cells["IdCategoria"].Value.ToString();
